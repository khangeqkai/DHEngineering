...
## Corpus Check
- 208 files · ~1,483,125 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 885 nodes · 1421 edges · 51 communities detected
...
- `autoAssignWorker()` --calls--> `recordHistory()`  [INFERRED]
  server/src/utils/timeEntryHelpers.js → server/src/db/helpers.js
...
Cohesion: 0.1
Nodes (19): autoAssignWorker(), checkCriticalInspection(), flagToBool(), isCriticalJob(), normalizeEmpty(), readFlag(), toBoolFlag(), toCamelCase() (+11 more)
...
Cohesion: 0.15
Nodes (15): bumpJobNumber(), getSettings(), peekNextJobNumber(), recordHistory(), updateSettings(), checkInterruptedRestore(), ensureBuiltInStandardLevel(), initializeDatabase() (+7 more)
...
Cohesion: 0.47
Nodes (3): LabourRatesSettings(), emptySchedule(), useLabourRates()
...
- **Thin community `Community 17`** (11 nodes): `Waves.jsx`, `Grad`, `.constructor()`, `.dot2()`, `Noise`, `.constructor()`, `.fade()`, `.lerp()`, `.perlin2()`, `.seed()`, `Waves()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 30`** (5 nodes): `maintenance.js`, `isMaintenance()`, `maintenanceGuard()`, `setMaintenance()`, `maintenance.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 38`** (5 nodes): `ScheduleEditor.jsx`, `DayRow()`, `ScheduleEditor()`, `segments()`, `toMin()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 39`** (4 nodes): `contacts.js`, `normalizeEmpty()`, `toApiFormat()`, `contacts.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 40`** (4 nodes): `machines.js`, `normalizeEmpty()`, `toResponseFormat()`, `machines.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 41`** (4 nodes): `tags.js`, `formatTag()`, `nameToValue()`, `tags.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 42`** (4 nodes): `auth.js`, `authenticate()`, `requireRole()`, `auth.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 47`** (3 nodes): `seed-tags.js`, `nameToValue()`, `seed-tags.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 48`** (3 nodes): `logger.js`, `logger.js`, `requestLogger()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 49`** (3 nodes): `LineItemTimerButton.jsx`, `formatElapsed()`, `LineItemTimerButton()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 50`** (3 nodes): `LineItemSupplierPicker.jsx`, `isActive()`, `LineItemSupplierPicker()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 51`** (3 nodes): `ScrapStat.jsx`, `formatNum()`, `ScrapStat()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `ApiService` connect `Community 0` to `Community 5`?**
