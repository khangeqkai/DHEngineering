...
Job card file endpoints (disk-first; one folder per category per job — no DB rows): All routes accept `category` ∈ `{ job-files, qa-form-files, customer-property-files }`, mapped to on-disk subfolders `Job Files`, `QA Forms`, `Customer Property` under `[base]/[Company]/[JobNumber]/`. `GET /jobcards/:id/files/:category` (authenticated, lists files), `GET /jobcards/:id/files/:category/:filename` (authenticated, returns file as base64), `POST /jobcards/:id/files/:category/from-scanner` (authenticated, copies scanner file into the folder; body: `{ filePath }` constrained to the configured scanner folder), `POST /jobcards/:id/files/:category/upload` (authenticated, saves base64 data; body: `{ filename, fileData }`). Filenames are timestamp-prefixed for collision-free storage and validated against extension allowlist + path-traversal guards. Base64 uploads are capped at 30 MB (raw bytes); larger payloads are rejected with 400.
...
**Per-line-item treatments**: Each job item carries a `treatments` JSON column (array of `{ value, otherText, supplierId, supplierName }`). `value` is a treatment tag value (or `'OTHER'` with required `otherText`); `supplierId` is required and chosen from suppliers whose `serviceTags` include the treatment. The UI is **one treatment + supplier per line item** via two linked native dropdowns (`tabs/LineItemTreatment.jsx`): picking a treatment filters the supplier list to those who offer it, and picking a supplier first filters the treatment list to what they offer (bidirectional). The `treatments` array therefore holds 0 or 1 entry, but stays an array so costing/PDF-fill/history/export code that iterates it is unchanged. Validated server-side in `validateItemTreatments` (which still accepts an array). This replaces the previous `subcontracts` table — there is no separate subcontract entity, tab, or workflow.
...
3. **Workers print** the pre-filled PDFs, fill inspection results by hand, then scan completed forms back via the Files menu in the JobCardModal header (Scanner / Camera / Upload → pick category → file written under the matching folder).
4. **All file uploads** (scanner, camera, base64) go through the unified `/jobcards/:id/files/:category` routes — same endpoints regardless of source, same disk layout regardless of category.
