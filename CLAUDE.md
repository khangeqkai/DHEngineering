...
- **Font**: Pragmatica (bundled OTF, weights 300/400/700), self-hosted. Nothing is fetched from a font CDN — the CSP in `client/index.html` forbids it, because workshop PCs are often offline and a remote font request just stalls the first screen
...
- **Workers change status only between In Progress and Material/Service** — and only while the job sits on Open, In Progress or Material/Service. Every other manual status change is management-only (In Progress and Done otherwise follow logged work). One rule, `canSetStatus` in `middleware/auth.js`, guards `PATCH /jobcards/:id/status` — the only status route a worker can reach, since `PUT /jobcards/:id` is management-only; the screen mirrors it from `jobcard/constants.js`.
...
- **The last active admin can't be demoted** — changing the only admin's role is refused, or nobody could ever grant costing access back.
...
- **Client error handling**: Use `toast.error()` from `react-hot-toast` (not `alert()`). **A check that belongs to a named form field marks that field instead of firing a pop-up** — `hooks/useFieldErrors.js` holds the messages (`groupClass`/`errorFor`, `fieldProps`/`errorProps` to wire a field's `id`/`aria-invalid`/`aria-describedby` to its message with one shared naming rule, plus `scrollFieldIntoView` to bring an off-screen field into view), `common/FieldError.jsx` renders the message under the field with `role="alert"`, and the two must never both fire for one failure. Server failures and anything not tied to a field stay pop-ups; a message that can repeat (failed status update, timer start/stop, customer list) takes a stable `id` so the second replaces the first rather than stacking; an operation slow enough to notice (exports, uploads, backup) shows a loading toast **during** the wait, on top of the disabled button
...
- **Icons are drawn, at three sizes.** Icons come from `lucide-react` at `size={14}` inline, `16` in controls and `18` in navigation; larger sizes are for empty states and headers only. A typed character (✓, ⚠, ✕, ▾) is never an icon — it renders differently on every machine and will not match the drawn icons beside it. Toasts in `use*.js` hooks take their icon from `common/toastIcons.jsx`, because plain `.js` files cannot hold JSX here. One spinner exists: `common/Spinner.jsx`.
- **The mouse wheel scrolls the page and does nothing else.** It never edits a value, anywhere in the app. The browser's default is the opposite: the wheel moves a focused `type="number"` input's value (and `range`/`date`/`datetime-local`/`month`/`time`/`week`, and a focused `<select>` in some browsers), which on a screen that writes itself rewrites a figure and saves it a second later with nobody having typed anything. This is enforced **once, app-wide**, by `utils/wheelValueGuard.js` — a single capturing, passive `wheel` listener installed from `main.jsx` that blurs the focused control when the pointer is over it, so the page still scrolls and the value cannot move. **Do not add an `onWheel` prop to an input**; there is no per-box guard any more and a second mechanism would be one more thing to forget. The old per-input version guarded the pricing sheet and had been forgotten on a part's quantity box — which is why it is no longer the component's job to remember.
- **Styles live in a `.css` file next to the component, never in a runtime `<style>` block.** A `<style>` tag inside a component only exists while that screen is mounted, so two screens could quietly define the same class differently and never collide. Real stylesheets all load at once, which is what turned the duplicated "show archived" checkbox and row action link into the single shared `.show-inactive-label` / `.row-link-btn` rules in `App.css`.
- **Every margin, padding and gap comes from the spacing scale — never a typed number.** `index.css` holds thirteen steps (`--space-0-5` 2px through `--space-12` 48px, 2px apart at the small end and 4px apart above 12) and, on top of them, five named intervals that say what the space is *for*: `--gap-inline` 6px (an icon and its label), `--gap-tight` 8px (controls acting as one cluster), `--gap-field` 12px (fields in a group), `--gap-group` 20px (groups in a section), `--gap-section` 32px (section to section). Reach for the named one first; a raw step is the fallback. The intervals widen as they go, and that widening is the whole point — it is what makes a section boundary read as bigger than a field gap instead of everything averaging out. Also `--pad-control`, `--pad-card`, `--pad-page`, `--pad-cell` for containers, and `--lead-heading` for the space *above* a heading, which is always more than the space below it. The app previously carried ~590 hand-typed values across 59 sizes; the only literals left are negative offsets that cancel a border or nudge an icon optically. A new number is a sign the scale needs a step, not that this rule has an exception.
- **One table density.** Every table in the app uses `--pad-cell`. There is no compact variant — the old `.table-compact` was deleted rather than left available to drift back to.
- **Two canonical breakpoints, 768px and 1024px.** There is no build step to hold them as variables, so they are written out at every call site — but every media query means one of those two widths. A component may reflow at another width when *its own* content runs out of room first, and the rule must carry a comment saying why. The full statement sits at the top of `index.css`; don't add a third without updating it.
- **Motion respects the reduced-motion preference, but a status signal slows instead of stopping.** `index.css` flattens every animation app-wide; the one spinner overrides that to a slow 2.5s turn, because it is the only sign that an export or upload is still running. Motion driven in JavaScript — the sign-in canvas — CSS can never reach, so it reads the preference itself and falls back to its still frame.
- **Screens other than sign-in and the job list load on demand.** `App.jsx` lazy-loads the rest behind a `Suspense` fallback inside the shell, so the sidebar and header stay put while the next screen arrives.
- **A data-entry control's border is `--border-input`, not `--border-color`.** The divider colour is only 1.25:1 against the surface, and a field's border is the only thing marking its edge, so it has to clear the 3:1 boundary minimum.
...
- **Work drives job status, and it overrides a manual change.** Invoiced is terminal and archives the job; it is refused while a timer runs, and an archived job's time can't be started, added, edited, resumed or deleted (stopping always works). → `jobs-and-status.md`
...
OPENROUTER_API_KEY=key       # Optional. Read from a git-ignored server/.env (see server/.env.example); absent in the packaged app
```
