...
- **Workers change status only between In Progress and Material/Service** — and only while the job sits on Open, In Progress or Material/Service. Every other manual status change is management-only (In Progress and Done otherwise follow logged work). One rule, `canSetStatus` in `middleware/auth.js`, guards both status-changing routes; the screen mirrors it from `jobcard/constants.js`.
