...
- **Employee (user) role**: Read-only job card view (Details tab renders as styled text, not inputs). Can only update photos via PUT. Tabs hidden from employees: Items, Subcontracts, Time, QA, Costing, Photos, Activity Log. Employees use Start/Stop timer for time tracking (one active timer at a time, enforced server-side). Can add notes but not delete them. **Quick Action Panel**: All users clicking a job card row see a QuickActionPanel (centered modal) with large buttons for Scan, Camera, Timer, View Documents, View Photos, and View Details. "View Documents" lists files from the job's QA Documents folder on disk with per-file View/Print actions. Clicking "View Details" opens the full JobCardModal. Active timers show a pulsing green indicator on job card rows for all users.
