...
Job card printout template endpoints (admin, in `jobcard-printout.js`, mounted under settings): `GET /settings/job-card-template` (returns `{ fileName, exists }`), `POST /settings/job-card-template` (`{ fileName, fileData(base64) }` — PDF only, 30 MB cap, saved to `[job_folders_base]/Job Card Template/`, filename stored in the `job_card_template` setting; replacing drops the prior file), `DELETE /settings/job-card-template`. There is ONE global printout template shared by every job.

Job card printout (per-job): `POST /jobcards/:id/print` (any authenticated user, in `jobcard-printout.js`) — fills the global printout template with the job's data via `fillPdfTemplate` (same field names as QA templates), saves it as a **single file at the top of the job's folder** (`Job Card {jobNumber}.pdf`, overwritten each print so it's never stale), and returns `{ fileName, filePath, data(base64), mimeType }` for printing (Electron `printFile` by path, or an in-page iframe print fallback). The printout is **not** a file category and is **never** part of the missing-attachment warning (the warning scan only reads the three category subfolders, so a loose file at the job folder root is invisible to it). 400s if no base path or no template configured.
...
**Job card printout vs quality forms (two separate features)**: These are deliberately split. (1) The **job card printout** is a single global fillable PDF template (uploaded once in Settings, stored at `[base]/Job Card Template/`), filled on demand with a job's data and saved as one file at the job folder root (`Job Card {jobNumber}.pdf`) for printing — a print-and-keep summary, never returned, never part of the missing-attachment warning. (2) **Quality forms** are per-QA-level inspection templates that are printed, hand-filled, and (for levels with `requires_returned_form` on) scanned back; only those levels drive the missing-quality-form warning. Both use the same `fillPdfTemplate` field names. See the printout endpoints above and the QA workflow below.

**Files & QA workflow**: All job card files live on disk, not in the database. Three category folders per job: `Job Files`, `QA Forms`, `Customer Property` (the job card printout is a loose file at the job folder root, not a category). The QA level system is the only producer of templated content:
...
3. **Workers print** the pre-filled PDFs, fill inspection results by hand, then bring the completed forms back in via the Files menu in the JobCardModal header (pick category → Choose File or Camera → file written under the matching folder). Whether a returned form is *required before invoicing* depends on the level's `requires_returned_form` switch (off = print-only, no nag).
