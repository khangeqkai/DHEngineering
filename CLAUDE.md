...
- **A number box that saves itself takes a wheel guard.** Chrome changes a focused `type="number"` input's value when the wheel goes over it, so on a screen that writes itself to the server without a Save button the scroll silently rewrites a figure. Attach `onWheel={(e) => e.currentTarget.blur()}` to those inputs — the pricing screen does it for every money and hours box (`CostingTab.jsx`). A number field behind an explicit Save doesn't need it.
